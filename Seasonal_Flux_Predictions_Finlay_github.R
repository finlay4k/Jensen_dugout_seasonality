...
gam.check(CO2_seasonal_gam, rep = 100)
layout(1)
summary(CO2_seasonal_gam)
...
summary(CH4_seasonal_gam)
gam.check(CH4_seasonal_gam, rep = 100)
...
k600.ch4.avg <- mean(calcs$`k600-CH4...89`)
CH4air.avg <- mean(calcs$CH4_uM_atm)

#calculate flux from predicted concentration using seasonal average k600 and CO2 air 

pred.ch4 <- pred.ch4 %>%
  mutate(Flux_dugout = (fitted-CH4air.avg)*k600.ch4.avg) %>% 
  mutate(CO2_equiv = Flux_dugout) 

# calculating co2 equivalence for CH4
pred.ch4 <- pred.ch4 %>% 
  mutate(CO2_equiv = ifelse(Flux_dugout > 0, 45*Flux_dugout, 203*Flux_dugout))

#sum CO2 equiv by site
co2eq.ch4.site <- pred.ch4 %>% group_by(Site_ID) %>% 
  summarise(Co2eqTotal = sum(CO2_equiv))
...
summary(N2O_seasonal_gls)
gam.check(N2O_seasonal_gls, rep = 100)
...
k600.n2o.avg <- mean(calcs$`k600-N2O...90`)
n2oair.avg <- mean(calcs$N2O_nM_atm)

#calculate flux from predicted concentration using seasonal average k600 and CO2 air 

pred.n2o <- pred.n2o %>% filter(group == "A") %>% 
  mutate(Flux_dugout = (fitted-n2oair.avg)*k600.n2o.avg) %>% 
  mutate(CO2_equiv = Flux_dugout) 

# calculating co2 equivalence for N2O
pred.n2o <- pred.n2o %>%
  mutate(CO2_equiv = ifelse(Flux_dugout > 0, 270*Flux_dugout, 349*Flux_dugout))

#sum CO2 equiv by site
co2eq.n2o.site <- pred.n2o %>% group_by(Site_ID) %>% 
  summarise(Co2eqTotal = (sum(CO2_equiv)/1000)) #convert umol to mmol to compare to CH4 and CO2 directly
...
write.csv(TotalCO2Equiv.gas, "TotalCO2Equiv.gas.csv")
...
#show the total per site with the contribution of each gas 
co2.eq.gas <-  ggplot(TotalCO2Equiv.gas, aes(x =factor(Site_ID, levels = c('23A', '32C', '56A', '32B', '4C', '4D', 
  ...
  annotate("text", x = 1, y = 10250, label = "c", size = 8)

co2.eq.gas            

require(cowplot)

all.fig.7 <- plot_grid(co2.eq.gas.july, co2.eq.gas.seasonal, co2.eq.gas, co2.eq.methods, ncol = 2, align = 'hv')
all.fig.7

jpeg("ByGas.jpeg", width = 16, height = 10, units = 'in', res = 1200)
plot(all.fig.7) 
dev.off()
...
flux <- calcs %>% select("Site_ID", "Date", "Flux_CO2_dugouts", "Flux_CH4_dugouts", "Flux_N2O_dugouts") %>% drop_na() %>% mutate(Date = ymd(Date)) %>% 
  group_by(Site_ID, Date) %>% 
  summarize(Flux_CO2_dugouts = mean(Flux_CO2_dugouts),
            Flux_CH4_dugouts = mean(Flux_CH4_dugouts),
            Flux_N2O_dugouts = mean(Flux_N2O_dugouts)) %>% 
  mutate(Month = month(Date)) %>% 
  mutate(CO2eq.co2 = Flux_CO2_dugouts, 
         CO2eq.ch4 = ifelse(Flux_CH4_dugouts > 0, 45*Flux_CH4_dugouts, 203*Flux_CH4_dugouts),
         CO2eq.n2o = (ifelse(Flux_N2O_dugouts> 0, 270*Flux_N2O_dugouts, 349*Flux_N2O_dugouts))/1000)

#calculate july only
#pull out july data
july.flux <- flux %>% filter(Month == 7)
july.flux.by.gas <- read.csv("july.by.gas.csv")
...
co2.eq.gas.july <-  ggplot(july.flux.by.gas, aes(x =factor(Site_ID, levels = c('23A', '32C', '56A', '32B', '4C', '4D', 
                                                                                    '61C', '4A','62C', '62B', '56B', '14B',
                                                                                    '66B', '32A', '66C', '68A', '14A', '61B',
                                                                                    '62E', '66A')), y = Seasonal, fill = ID))+ #reorder the sites to match the first panel
  geom_bar(position = 'dodge', stat = "identity", alpha = 0.75, color = 'black')+
  ylab(expression("Total Seasonal CO"[2]*'-eq Flux (mmol m'^2*")")) +
  SAE_theme +
  xlab("Site") +
  ...
  theme(legend.position=c(0.1, 0.88),
        legend.background = element_blank()) +
  annotate("text", x = 1, y = 50000, label = "a", size = 8)
...
SeasonalFluxByGas <-read.csv("SeasonalFluxByGas.csv")

co2.eq.gas.seasonal <-  ggplot(SeasonalFluxByGas, aes(x =factor(Site_ID, levels = c('23A', '32C', '56A', '32B', '4C', '4D', 
                                                                        '61C', '4A','62C', '62B', '56B', '14B',
                                                                        '66B', '32A', '66C', '68A', '14A', '61B',
                                                                        '62E', '66A')), y = CO2.eq.year, fill = ID))+ #reorder the sites to match the first panel
  geom_bar(position = 'dodge', stat = "identity", alpha = 0.75, color = 'black')+
  ylab(expression("Total Seasonal CO"[2]*'-eq Flux (mmol m'^2*")")) +
  SAE_theme +
  xlab("Site") +
  ...
  theme(legend.position=c(0.1, 0.88),
        legend.background = element_blank()) +
  annotate("text", x = 1, y = 90000, label = "b", size = 8)
...
#spring - 30 days between start of may sampling and mid-point between may and seasonal sampling (May 15 to Jun 13)
...
write.csv(compare.methods, "Compare.methods.csv" )
...
co2.eq.methods <-  ggplot(compare.methods, aes(x =factor(Site_ID, levels = c('23A', '32C', '56A', '32B', '4C', '4D', 
  ...
  theme(legend.position=c(0.2, 0.88),
        legend.background = element_blank()) +
  annotate("text", x = 1, y = 90000, label = "d", size = 8)

co2.eq.methods

#Stacked bar graph of gases contributing to CO2eq flux, by method
StackedBar <-read.csv("StackedBarData.csv")

co2.eq.methods.stacked.july <-  ggplot(StackedBar, aes(x =factor(Site_ID, levels = c('23A', '32C', '56A', '32B', '4C', '4D', 
                                                                             '61C', '4A','62C', '62B', '56B', '14B',
                                                                             '66B', '32A', '66C', '68A', '14A', '61B',
                                                                             '62E', '66A')), y = CO2.eq.year.july, fill = ID))+ #reorder the sites to match the other CO2 equiv plots from lines 191 and 202
  ...
  theme(legend.position=c(0.2, 0.88),
        legend.background = element_blank()) +
  annotate("text", x = 1, y = 50000, label = "a", size = 10)

co2.eq.methods.stacked.july

co2.eq.methods.stacked.seasonal <-  ggplot(StackedBar, aes(x =factor(Site_ID, levels = c('23A', '32C', '56A', '32B', '4C', '4D', 
                                                                                     '61C', '4A','62C', '62B', '56B', '14B',
                                                                                     '66B', '32A', '66C', '68A', '14A', '61B',
                                                                                     '62E', '66A')), y = CO2.eq.year.seasonal, fill = ID))+ #reorder the sites to match the other CO2 equiv plots from lines 191 and 202
  geom_bar(position = 'stack', stat = "identity", alpha = 0.75, color = 'black')+
  ylab(expression("Total Seasonal CO"[2]*'-eq Flux (mmol m'^2*")")) +
  SAE_theme +
  xlab("Site") +
  ...
  theme(legend.position=c(0.2, 0.88),
        legend.background = element_blank()) +
  annotate("text", x = 1, y = 90000, label = "b", size = 10)

co2.eq.methods.stacked.seasonal

co2.eq.methods.stacked.gam <-  ggplot(StackedBar, aes(x =factor(Site_ID, levels = c('23A', '32C', '56A', '32B', '4C', '4D', 
                                                                                         '61C', '4A','62C', '62B', '56B', '14B',
                                                                                         '66B', '32A', '66C', '68A', '14A', '61B',
                                                                                         '62E', '66A')), y = CO2.eq.year.gam2, fill = ID))+ #reorder the sites to match the other CO2 equiv plots from lines 191 and 202
  geom_bar(position = 'stack', stat = "identity", alpha = 0.75, color = 'black')+
  ylab(expression("Total Seasonal CO"[2]*'-eq Flux (mmol m'^2*")")) +
  SAE_theme +
  xlab("Site") +
  ...
  theme(legend.position=c(0.2, 0.88),
        legend.background = element_blank()) +
  annotate("text", x = 1, y = 50000, label = "c", size = 10)

co2.eq.methods.stacked.gam

require(cowplot)

Co2.eq.methods.all <- plot_grid(co2.eq.methods.stacked.july, co2.eq.methods.stacked.seasonal, co2.eq.methods.stacked.gam, nrow = 3, align = 'hv')
Co2.eq.methods.all

jpeg("Co2.eq.methods.all.jpeg", width = 7, height = 15, units = 'in', res = 1200)
plot(Co2.eq.methods.all) 
dev.off()
...
pred.co2.upper <- pred.co2 %>%
  mutate(Flux_dugout = (upper-CO2air.avg)*k600.co2.avg) %>% 
  mutate(CO2_equiv = Flux_dugout) 
...
pred.ch4.upper <- pred.ch4 %>%
  mutate(Flux_dugout = (upper-CH4air.avg)*k600.ch4.avg) %>% 
  mutate(CO2_equiv = Flux_dugout) 
...
pred.n2o.upper <- pred.n2o %>% filter(group == "A") %>% 
  mutate(Flux_dugout = (upper-n2oair.avg)*k600.n2o.avg) %>% 
  mutate(CO2_equiv = Flux_dugout) 

#sum CO2 equiv by site
co2eq.n2o.site.upper <- pred.n2o.upper %>% group_by(Site_ID) %>% 
  summarise(Co2eqTotal = (sum(Flux_dugout)/1000)) #convert umol to mmol to compare to CH4 and CO2 directly
...
pred.n2o.lower <- pred.n2o %>% filter(group == "A") %>% 
  mutate(Flux_dugout = (lower-n2oair.avg)*k600.n2o.avg) %>% 
  mutate(CO2_equiv = Flux_dugout) 
