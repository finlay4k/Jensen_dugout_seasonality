...
#flux figures
CO2 <- ggplot(master, aes(Month, CO2Flux_cc, color = Site_ID)) +
  geom_point(size = 3) + geom_line(aes(Month, CO2Flux_cc, color = Site_ID)) +
  labs(y=expression(CO[2]*" Flux (mmol m"^-2*" day"^-1*")")) +
  xlab("Month") +
  SAE_theme +
  theme(legend.position = "none") +
  scale_x_discrete(name = element_blank(), 
                   limits=c("April","May","June", "July", "August", "September"),
                   labels=c("Apr.","May","June", "July", "Aug.", "Sept.")) +
...
CH4 <- ggplot(master, aes(Month, CH4Flux_cc, color = Site_ID)) +
  geom_point(size = 3) +
  labs(y=expression(CH[4]*" Flux (mmol m"^-2*" day"^-1*")")) +
  xlab("Month") +
  SAE_theme +
  theme(legend.position = "none") +
  scale_x_discrete(name = element_blank(), 
...
N2Ogap <- ggplot(master, aes(Month, N2OFlux_cc, color = Site_ID)) +
  geom_point(size = 3) +
  labs(y=expression(N[2]*"O Flux ("*mu*"mol m"^-2*" day"^-1*")")) +
  SAE_theme +
  theme(legend.position = "none") +
  ...
  stat_summary(geom = "point",
               ...
               shape = 22,
               fill = "black") +
  facet_zoom(ylim = c(-15, 35), zoom.size = 1)

N2Ogap

require(cowplot)

all <- plot_grid(CO2, CH4, N2Ogap, ncol = 1, align = 'hv')
all

ggsave("Flux_Figure_PointPlot.png", all, height = 10, width = 7.5, units = 'in')

#ebullition figure and anova
bubbles <- read_xlsx("2018BubbleTrapData.xlsx", sheet = 'Sheet2')

CH4 <- ggplot(bubbles, aes(x = Site, y = CH4mmol.m2.d )) + geom_point(color = "grey", size = 3) + 
  SAE_theme +
  stat_summary(geom = "point", fun.y = "mean", col = "red", size = 3, shape = 22, fill = "red") + 
  scale_x_discrete(name ="Depth", 
                   limits=c("Shallow", "Deep")) +
  labs(y = expression(paste("CH"[4]," Ebullitive Flux (mmol ", m^-2, " ", d^-1, ")"))) +
  scale_y_continuous(lim = c(-0.1, 1.51), breaks = c(0, 0.5, 1, 1.5))
CH4

ch4 <- aov(CH4mmol.m2.d ~ Site, data = bubbles)
summary(ch4)

bubbles %>% 
  group_by(Site) %>% 
  summarize(mean = mean(CH4mmol.m2.d),
            stdev = sd(CH4mmol.m2.d))